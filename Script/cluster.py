...
'''
class Cluster:
    def __init__(self, contig_info, contig_local, dict_contigRev, dict_contigRevLocal, dict_contig_len, contig_markers,
                 normcc_matrix, imputed_matrix, bins, bin_of_contigs, bac_mg_table, ar_mg_table, path, intra, inter, cont_weight, 
                 min_quality, min_comp, max_cont, report_quality, min_binsize):
        ...
        logger.info('The best parameter for nofix is {} with {} high-quality MAGs'.format(res_para_nofix, max(total_5_nofix)))
        ...
        self.bin2contig_nofix_decontamin = self.decontamin(bin2contig_nofix_ori)
        ...
        for i in self.bin2contig_nofix_decontamin.values():
            ...
            if comp >= 50 and cont <= 5:
                num_50 += 1
        logger.info('After the decontamination, there are {} high quality MAGs for nofix'.format(num_50))
        ...
        for res in res_option:   
            ...
            del new_membership, optimiser, mem, part_fix, cluster_fix, temp
        ...
        logger.info('The best parameter for prefix is {} with {} high-quality MAGs'.format(res_para_fix, max(total_5_fix)))
        ...
        for i in self.bin2contig_fix_decontamin.values():
            ...
            if comp >= 50 and cont <= 5:
                num_50 += 1
        logger.info('After the decontamination, there are {} high quality MAGs for prefix'.format(num_50))
        ...
        SEL_MIN_QUALITY = min_quality  # add_argument('-q', '--sel_min_quality', help="minimum quality of bin to consider during bin selection process", type=float, default=50)
        ...
        while True:
            ...
            matched_sets = ensemble._matched_bin_sets(bins_combine,
    ...
    def decontamin(self, bin2contig_ori):
        ...
        for _contam_bin in _contam_bins:
            ...
            for _contig in _contig_in_contam:
                ...
                if _contig in self.contig_markers:
            ...
            for i in _contig_info_all_contam[: , 0]:
                if i in _bin_of_contigs_contam:
                    ...
                    _new_membership.append(np.random.randint(0,high=len(_bins_contam)))
